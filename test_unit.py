...
def test_is_even():
    ...
    print("Тестируем проверку четности...")
    assert math_operations.is_even(4) == True, "4 должно быть четным"
    assert math_operations.is_even(7) == False, "7 должно быть нечетным"
    assert math_operations.is_even(0) == True, "0 должно быть четным"
    print("Проверка четности работает корректно")

def test_factorial():
    """Тестирование функции факториала"""
    print("Тестируем факториал...")
    assert math_operations.factorial(0) == 1, "0! должно быть 1"
    assert math_operations.factorial(1) == 1, "1! должно быть 1"
    assert math_operations.factorial(5) == 120, "5! должно быть 120"
    
    try:
        math_operations.factorial(-1)
        assert False, "Факториал отрицательного числа должен вызывать ошибку"
    except ValueError as e:
        assert str(e) == "Факториал отрицательного числа не определен", "Неверное сообщение об ошибке"
    print("Факториал работает корректно")
